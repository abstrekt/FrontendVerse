...
5. Pick next globally unique id(s) for JS Learnings via `pnpm run validate:learning-ids --next-id` (or a reserved pair id when safe). For other sections, use next id in the target file. Only for items that will actually be **added** (not updated/merged).
